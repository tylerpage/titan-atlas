...
const usesMetaAds = computed(() => form.connector_type === 'meta_ads');
const usesAmazonAds = computed(() => form.connector_type === 'amazon_ads');
const usesWalmartConnect = computed(() => form.connector_type === 'walmart_connect');
const usesEbayAds = computed(() => form.connector_type === 'ebay_ads');
...
const lastDefaultConnectionName = ref('');
...
watch(
    ...
    (type) => {
        ...
        const defaultName = connector?.label ?? '';

        if (!form.name || form.name === lastDefaultConnectionName.value) {
            form.name = defaultName;
        }

        lastDefaultConnectionName.value = defaultName;
...
function submit() {
    ...
    if (usesMetaAds.value && !form.credentials.ad_account_id) {
        form.setError('credentials.ad_account_id', 'Select a Meta ad account after testing your access token.');

        return;
    }

    if (usesAmazonAds.value && !form.credentials.profile_id) {
        form.setError('credentials.profile_id', 'Select an Amazon Ads profile after testing your access token.');

        return;
    }

    if (usesWalmartConnect.value && !form.credentials.advertiser_id) {
        form.setError('credentials.advertiser_id', 'Select a Walmart Connect advertiser after testing your access token.');

        return;
    }

    if (usesEbayAds.value && !form.credentials.account_id) {
        form.setError('credentials.account_id', 'Select an eBay ad account after testing your access token.');
